...
DNF_2D::DNF_2D(int _dimensions1, int _dimensions2, bool debug) {
    ...
    // Initialize tensors
    m_activation1 = torch::zeros({m_dimensions1});
    m_activation2 = torch::zeros({m_dimensions2});
    ...
    m_interaction_kernel = torch::zeros({m_dimensions1, m_dimensions1}); // TODO: Get sizes for the kernels from the DNF constructor
...
void DNF_2D::step(torch::Tensor input1, torch::Tensor input2, float dt) {
    ...
    m_output = m_activation1.unsqueeze(1).matmul(m_activation2.unsqueeze(0));
}

// Getters
torch::Tensor DNF_2D::get_activation(int index_activation) {
...
// Setters
void DNF_2D::set_activation(torch::Tensor activation1, torch::Tensor activation2) {
    m_activation1 = activation1;
    m_activation2 = activation2;
}
